...
describe("ImageSelector", () => {
  it("doit afficher le champ URL et le bouton de sélection", () => {
    ...
    expect(
      screen.getByPlaceholderText("Entrez l'URL de l'image..."),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Sélectionner un fichier local"),
    ).toBeInTheDocument();
  });
...
describe("AddAccountModal", () => {
  ...
  it("envoie les tags en tableau (split/trim/filter)", () => {
    const onAdd = vi.fn();
    const onClose = vi.fn();
    render(
      <AddAccountModal
    ...
    fireEvent.change(screen.getByPlaceholderText("smurf, main, soloq"), {
      target: { value: " main , smurf ," },
    });
    fireEvent.click(screen.getByText("Ajouter le compte"));
    expect(onAdd).toHaveBeenCalledWith(
      expect.objectContaining({ tags: ["main", "smurf"] }),
    );
  });
