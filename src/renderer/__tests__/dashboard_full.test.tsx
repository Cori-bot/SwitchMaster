...
describe("Dashboard Component", () => {
  ...
  describe("drag and drop", () => {
    it("doit échanger les items et appeler onReorder au drop", async () => {
      ...
      await act(async () => {
        fireEvent.dragStart(card1, {
          dataTransfer: { setDragImage: vi.fn(), setData: vi.fn() },
        });
      });
    ...
    it("ne doit pas échanger si les conditions de position ne sont pas remplies", async () => {
      ...
      const calledIds =
        defaultProps.onReorder.mock.calls[
          defaultProps.onReorder.mock.calls.length - 1
        ][0];
  ...
  describe("interactions", () => {
    it("doit appeler onSwitch quand on clique sur switch", () => {
      ...
      // Dashboard relaie via onPreSwitch(id, autoLaunch?) ; le 2e arg est
      // ici undefined car la mock AccountCard n'appelle onSwitch qu'avec l'id.
      expect(defaultProps.onSwitch).toHaveBeenCalledWith("1", undefined);
    });
