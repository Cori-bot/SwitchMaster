...
function GameView({
  ...
  const [clientPath] = useState<string>(
    isRiotClient && riotPath ? riotPath : getFakePath(wallpaper.title),
  );
  ...
  useEffect(() => {
    const updateButtonPosition = () => {
      if (formContainerRef.current && backBtnRef.current) {
        ...
        backBtnRef.current.style.setProperty(
          "top",
          `${topPosition}px`,
          "important",
        );
  ...
  return (
    <div className={`wallpaper-view ${isClosing ? "closing" : "opening"}`}>
      ...
      <button ref={backBtnRef} className="back-btn" onClick={onClose}>
        ‹
      </button>
